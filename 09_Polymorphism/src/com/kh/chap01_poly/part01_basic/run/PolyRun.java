...
public class PolyRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 1. 부모타입 레퍼런스 변수로 부모타입 객체를 다루는 것 = 당연히 가능하다
		 * 
		 */
		
		
		
		System.out.println("1. 부모타입 레퍼런스 변수로 부모 객체를 다루는 경우");
		...
		Child1 c1 = new Child1(); // 기본생성자에 슈퍼생성자 호출하는 구문이 항상 있음. 따라서 부모객체 사용가능.
		c1.printChild1();
		((Parent)c1).printParent();
		//c1 레퍼런스 Parent(부모), Child1(자식)에 있는 필드나 메서드 둘다 접근가능.
		...
		System.out.println("3. 부모 타입 레퍼런스로 자식 객체를 다루는 경우(다형성 적용된 개념)");
		Parent p2  = /*(Parent) */ new Child1();
		/*
		 * 양쪽의 자료형이 다른데도 불구하고 오류 발생 X
		 * Parent형으로 '자동형변환' 되고 있기 때문에 
		 * 상속 구조에서는 클래스 형변환이 가능하다.
		 * 
		 */
		
		p2.printParent(); // p2라는 레퍼런스 변수로는 Parent에만 접근이 가능.
		((Child1) p2).printChild1(); // 단, 자식객체로 접근하고 싶을때 Child1형으로 강제형변환후 Child1에 접근가능.
		 ...
		 * 
		 * 다형성을 쓰는 이유
		 * 	- 부모타입으로부터 파생된 여러가지 타입의 지식객체들을 부모클래스 타입 하나로 다룰수 있음.
		 * 
		 * 
		 */
		...
		// 단, 다형성이 적용되면 부모타입 레퍼런스 변수로 모든 자식객체를 다 받아줄수 있다.
		...
		// 하나의 부모타입으로 모든 자식객체들을 받을수 있음 => 편리, 코드수 감소, 생산성이 증대.
