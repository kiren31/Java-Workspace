...
public interface Basic {
	 ...
	 *    다중상속(구현)이 가능함.)
	 * - 추상 클래스와 다르게 좀더 강한 규칙성과 강제성을 가지는게 인터페이스이다.
	 * 
	 * 추상클래스와 인터페이스 
	 * 
	 * 1. 공통점
	 *  - 객체 생성은 안되나 , 참조변수로서 사용이 가능하다. (즉 ,다형성을 적용할 수 있따.)
	 *  - 상속(구현)하는 클래스에 추상메소드를 구현하도록 강제한다.
	 *  
	 * 2. 차이점
	 *  - 추상클래스는 클래스 내에 인스턴스 변수 ,메소드를 생성이 가능하고, 추상메소드가 포함되었거나
	 *    abstract키워드로 클래스가 정의되어 있고
	 *    인터페이스는 인스턴스 변수, 메소드 생성이 불가능하며 모든 변수는 상수필드, 모든 메소드는 추상메소드로 정의되어있따.
	 * 
	 *  - 존재하는 목적이 다르다.
	...
	void sleep();
	// 인터페이스에서는 추상메소드만 정의할 수 있기때문에 메소드의 몸통부 작성이 불가능하며
