...
public class A_Array {
	...
	public void method1() {
		 ...
		 * 
		 * 2) 배열 할당
		 * 이 배열에 몇개의 값들을 보관할건지 배열의 크기를 지정 해주는 과정
	...
	// 각각의 인덱스 값을 확인하기 위해서는 [] 블럭을 통해 내가 확인 하고자 하는 인덱스(방번호)를 제시해줘야함.
	...
	public void method2() {
		 ...
		 * 5칸짜리 방이 만들어진 후에 각방에 기본값으로(int 0으로)데이터가 초기화 되고, 주소값이 할당됨(0x123)
		 * 마지막으로 대입 연산자가 실행되면서 주소값이 arr에 할당이 됨.
		 */
		 ...
		 * => 실제 값(리터럴)을 바로 stack 영역에서 담을 수 있는 변수 => 일반 변수
		 * 
	...
	public void method3() {
		 ...
		 * -> heap이라는 영역에는 절대로 빈 공간이 존재 할 수 없기때문.
	...
	public void method5() {
		...
		int[] arr = null; // null값 할당 가능. 주소값을 담을 수 있는 변수의 기본값은 null;
	...
	public void method6() {
		...
		/*
		 * 배열의 가장 큰 단점 
		 * 한번 지정한 배열의 크기는 변경 불가함.
		 * => 배열의 크기를 변경하고자 한다면 어쩔수 없이 새로운 배열을 다시 만들어야함.
		 */
		 ...
		 * 일정 시간이 지나고서도, 사용이 되지 않으면 "가비지 컬렉터(GC)" 가 삭제 시켜줌.
		 * (자동 메모리 관리)
		 * => 자바에서 자동 메모리 관리 특징. 개발자가 코드에만 신경 쓸 수 있게 해줌.
		 */
	...
	public void method7() {
		...
		// arr1 -> arr1의 heap 영역의 "주소값"
		// arr2 -> arr2의 heap 영역의 "주소값"
		// 주소값 == 주소값 : 주소값간에 동등 비교를 수행하기 때문에 false
		
	}
	
	public void method8() {
		//1. 크기 10짜리 정수(int) 배열 선언.
		//2. 반복문을 활용해서 0번 인덱스 부터 마지막 인덱스까지 순차적으로 접근하면서
	...
	public void method9() {
		...
		//2. 반복문을 활용해서 매번 사용자에게 과일명을 입력받아 그 값을 매~ 인덕스자리에 대입
	...
	public void method11() {
		// 1. 사용자에게 배열의 길이를 입력 받은 후, 해당 크기 만큼 정수 배열 생성
		Scanner sc = new Scanner(System.in);
