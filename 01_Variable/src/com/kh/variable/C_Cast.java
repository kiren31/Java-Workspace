...
public class C_Cast {
	 ...
	 * 			  => 직은 바이트의 자료형 - > 큰바이트의 자료형
	 * 
	 * 2. 강제(명시적) 형변환 => 자동형변환이 되지 않아 내가 직접 강제로 형변환을 해야하는경우
	...
	public void autoCasting() {
		...
		// 아무리 4byte float라고 해도 long형보다 담을수 있는 값의 범위가 더 크기 때문에
		// float형이 실수이기 때문에 long형보다 표현가능한 범위의수가 더 커서 자동형변환이 이루어짐.
		...
		// 4. 특이케이스 chat(2byte) <-> int(4byte) : 양방향 형변환 가능.
		...
		/*
		 * char의 범위 0~65xxx : 각 문자마다 고유의 정수값이 정해져있다. 따라서 쌍방형 변환이 
		 * 가능하다. 음수값은 오류가 난다.
		...
		char sum = 'A'+10 ; //65 문자와 정수사이에 연산이 가능하다.
		...
		//byte로 연산시 연산결과의 정수값은무조건 int로 취급한다.
	...
	public void forceCasting() {
		...
		//해결방법 2. 덧셈 연산전에 dNum 변수를 미리 int형으로 강제 형변환
		//			자동형변환은 작은 바이트변수가 큰 바이트 변수로 형변환 되기 때문에
		//			큰 바이트인 dNum 변수를 강제로 int형으로 변환시킨것.
