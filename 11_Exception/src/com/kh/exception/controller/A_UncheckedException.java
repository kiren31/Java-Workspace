...
public class A_UncheckedException {
	 ...
	 * - ArrayIndexOutOfBoundsException : 배열의 부적절한 인덱스로 접근할때 발생하는 예외 : 6 , -1
	 * - NegativeArraysSizeException : 배열의 크기를 음수로 지정할 경우 발생하는 예외  new int[x]; : -5
	 * - ClassCastException : 허용할 수 없는 형변환이 진행될 경우 발생하는 예외 : (Object)
	 * - NullPointerException : 레퍼런스가 아직 null임에도 불구하고 접근하려고 할때 발생하는 예외
	 * - ArithmeticException : 나누기 연산시 0으로 나눌때 발생하는 예외 
	 * - .....
	 * 
	 * => 이러한 RuntimeException 과 관련한 예외는 충분히 예상가능한 상황이기 때문에
...
//			int [] arr =  new int[Math.abs(size)]; // 방법 2. 항상 양의 정수값이 들어갈수있도록 abs메서드 사용
//		
//		}
...
//					+ "인덱스로 접근한 것 같습니다.");
//		} 	// 다중 catch 블럭이 줄어들었음. 정확히 어떤 에러인지 알수없음.
		...
		try {
			...
			System.out.println("예외가 발생하긴 했네요. 근데 배열의 크기를 음수로 입력했거나, 부적절한"
					+ "인덱스로 접근한 것 같습니다.");
		} // 다중 catch문 에서는 블럭의 순서가 중요함. => 범위가 작은 자식타입의 예외 클래스를 먼저 기술해야함.
	}
	
	/*
	 * RuntimeException 관련된 예외는
	 * - 조건문으로 해결 가능 => 예외 자체가 발생 안되게끔 개발자가 소스코드로 핸들링 가능
	 * = 예외처리 구문으로도 해결 가능 => 예외가 발생했을대를 대비해서 그 때 실행할 내용을 정의해 두는것.
	 * 
