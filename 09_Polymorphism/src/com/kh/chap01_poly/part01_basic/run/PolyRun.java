...
public class PolyRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 1. 부모타입 래퍼런스 변수로 부모타입 객체를 다루는것 => 당연히가능하다
		 */
		...
		//c1 레퍼런스로 Parent(부모), Child1(자식) 에있는 필드나 메서드 둘다 접근가능.
		//c1레퍼런스변수가 Parent에 있는 메소드에 접근시 자동으로 현변환된채 진행됨.
		//즉 자식에서 부모클래스 자동형변환(업캐스팅). -> 작은것에서 큰것으로 형변환 된것과 같은이치
		...
		((Child1) p2).printChild1();// 단, 자식객체로 접근하고싶을때 Child1형으로 강제형변환후 Child1에 접근가능.
		...
		for(int i =0; i < arr.length; i++) {
			...
			// 현재 참조하고있는 레퍼런스 변수가 실제로 어떤 클래스(객체)타입을 참조하고 있는지 확인할때 사용
...
//			}
			 ...
			 * 단, 참조하고 있는 자식클래스에 해당 메소드가 오버라이딩 되어 있따면
