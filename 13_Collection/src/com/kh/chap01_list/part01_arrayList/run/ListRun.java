...
public class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 방대한 데이터들을 단지 담아만 두고 조화만 할 목적 => 배열
		 * 방대한 데이터들이 빈번하게 추가 , 삭제, 수정 될 것 같다 => 컬렉션
		 * 
		 * 컬렉션의 3가지 분류
		 * 
		 * List 계열 => 담고자 하는 값(value)만 저장 / 값 저장 시 순서 유지됨(index의 개념이 있음) / 중복값 허용.
		 * 				ex) ArraList, Vector, LinkedList \ > ArrayList가 제일 많이 쓰임(90% 이상)
		 * 
		 * Set 계열  => 담고자 하는 값(value)만 저장 / 값 저장 시 순서 유지 X(index 개념이 없음) / 중복값 허용 X
		 * 				ex) HashSet, TreeSet
		 * 
		 * Map 계열  => 키(Key) + 담고자 하는 값(value)세트로 저장 / 값 저장 시 순서 유지 X(index 개념 없음)
		 * 				/ Key 중복 허용 X
