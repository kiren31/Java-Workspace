 ...
 * - 지역변수 : 클래스 영역 내의 특정 영역({})에 선언한 변수(선언한 영역에서만 사용가능)
 ...
 * 	  소멸시점 : 객체 소멸시 같이 소멸
 * 
 * 	 -클래스변수 == static변수 : static이라는 예약어가 붙어있냐 아니냐에 따라 멤버변수냐 클래스 변수냐로 구분됨.
...
public class FieldTest1 {// FieldTest1 클래스 시작
	...
	public void test(int num) {// test메소드 시작
		...
		//지역변수(메소드 영역 내에 생성.) => 지역변수에는 접근제한자 붙히지 않음.
		...
		/*
		 * 멤버 변수 출력시 에러가 안나는이유
		 * 해당 클래스 객체 생성시 jvm이 값을 해당필드로 자동으로 대입시켜줬기 때문.
		 * 물론, 명시적 초기화도 가능함.
		 */
