...
public class DateRun {
	...
	public static void main(String[] args) {
		 ...
		 * - 자바 개발 초창기에 급하게 만들어서 완성도가 높지 않은 클래스(다국적으로 쓰기 적합 X)
		 * 	
		 */
		
		// 기본생성자를 통해 Date 객체 생성 => 현재 날짜 및 시간을 가지고옴.
		Date today = new Date();
		...
		// 1. 형식을 지정하면서 SimpleDateFormat 객체 생생. %d %f %c %s
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy년 MM월 dd일 hh시 mm분 ss초");
		
		
		
		
		// 2. sdf 객체에서 제공하는 format 메소드 호출 하면서 ~ Date객체 전달 => 포맷에 지정한 형식대로 String 문자열 반환
		// 	  sdf.format(Date 객체) : String
