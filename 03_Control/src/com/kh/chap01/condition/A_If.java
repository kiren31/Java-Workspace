...
public class A_If {
	 ...
	 * 
	 * 조건문은 크게 if문(단독, if~else, if~else if, 중복 if) switch 문으로 나뉨
	 */
	// 삼항 연산자를 조건문으로 바꿔보기
	public void method1() {
		/*
		 ...
		 * 	 조건식이 true인경우 실행할 코드
		 * }
		 * => 조건시의 결과가 참일 경우 중괄호 블록 안의 코드가 실행.
...
//	}
	...
	public void method5() {
		Scanner sc = new Scanner(System.in);
		//사용자에게 이름, 성별을 읿력받아 남자인지, 여자인지 출력하는 메서드
		...
		if(gender == 'm' || gender == 'M') {
			...
			// return이라는 녀석을 사용
			 return; //해당 일반 메서드를 빠져나김(메서드를 호출했던곳으로 돌아감)
		}
		
		// XXX님은 XXX이다.
		System.out.printf("%s님은 %s이다.", name, result);
		// -> 실행시 오류남. 위에 String result = "";로 초기화 한번해두는게 필요
	}
	
	public void method6() {
		// 특이 케이스. 문자열간의 동등비교 ( 문자열 == 문자열)
		 ...
		 * 참조 자료형: String / 특이한녀석
		 * 
		 * 기본 자료형간 비교시 == != 사용가능하지만, 참조 자료형은 동등 비교연산시 정상적으로 연산이 안됨.
		 * 그럼 어떻게 하냐? 문자열.equals() 라는 방법으로 비교함.
		 * 
		 * 
		 */
