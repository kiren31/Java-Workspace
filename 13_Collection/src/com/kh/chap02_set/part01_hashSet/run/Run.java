...
//		hs2.add(new Student("민경민",31,100));
		...
		// equals() 한 결과가 true이고 hashCode()값이 일치하면 => 동일 객체로 판단 (중복저장 X)
		
		// Object클래스에 있는 equals() : 두 객체의 주소값을 가지고 비교해서 일치하면 true / 일치하지 않으면 false
		// Object클래스에 있는 hashCode() : 두 객체의 주소값을 기반으로 해서 해시 코드값을 만들어서 변환하여 비교.
		...
		// Student 클래스에 hashCode() 오버라이딩 : 세 필드값 기반으로 해서 해시코드값을 만들어서 반환.
		...
		while(it.hasNext()) { // it에 next로 빼올 값이 있는가? 라고 물어보는 메소드
			it.next();
		} // String 의 StringTokenizer와 비슷한 개념.
