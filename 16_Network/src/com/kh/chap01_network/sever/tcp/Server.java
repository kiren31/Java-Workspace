package com.kh.chap01_network.sever.tcp;
...
public class Server {
	...
	public void serverStart() {
		...
		try {
			...
			while(true) {
				//3) 접속요청이 오면 요청 수락(accept()후 해당 클라이언트의 대한 소켓객체 생성
				//4) 클라이언트의 정보를 저장.
				System.out.println("클라이언트 대기중...");
				Socket client = server.accept(); // Socket -> 클라이언트와 통신할수있는 객체.
				...
				// 7) 스트림을 통해 읽고 쓰기.
				while(true) {
					String message = br.readLine(); // 클라이언트로부터 입력이 있기전까지 멈춤.
