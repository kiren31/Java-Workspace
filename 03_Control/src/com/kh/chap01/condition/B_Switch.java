...
public class B_Switch {
	 ...
	 * switch 조건식 => 정확한 값의 조건만 기술(동등 비교만 수행)
	 * 
	 * [표현법]
	 * switch(앞으로 동등 비교를 할 대상자){
	 * case 값1 : 실행할 코드1; // 앞으로 동등 비교를 할 대상자 == 값1일 경우 실행할 코드.
	 * 			  break;	// 실행할 코드1만 실행하고 switch문을 빠져나갈지 결정하는 키워드
	 * 	// 브레이크문이 없다면 이아래코드도 계~속 실행됨.
	 * case 값2 : 실행할 코드2;
	 * 			  break;
	 * case 값n : 실행할 코드n;
	 * 			  break;
	 ...
	 * 					  // 위의 값1, 값2, 값n... 앞으로 동등 비교를 할 대상과 하나도 일치하지
	 * 					  // 않다면, default내의 실행할 코드가 실행된다.
	 * }
	 * 
	 * switch문 자주 사용되진 않지만, 사용하는 경우는 정말 정밀하게 동일한 연산결과가 수행되어야 하는경우
	 * ex) 키보드 입력, 마우스 입력등.
	 */
