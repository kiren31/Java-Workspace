...
public class ArrayForeach {
	...
	public static void main(String[] args) {
		...
		for(int num : arr2) { 
			// arr2라는 배열은 0번 인덱스부터 순차적으로 접근하면서 int num 이라는 변수에 
			// arr2의 각 인덱스의 값을 저장함.
		...
		System.out.print("구매하려는 핸드폰의 이름을 입력하세요 : ");
		String serchphone = sc.nextLine();
		
		int count = 0;
		
		for(Phone phone : phones) {
			if(phone.getSeries().equals(serchphone)) {
				System.out.println(serchphone+"핸드폰의 가격은"+phone.getPrice()+"입니다.");
				count++;
				break;
			}
			if(count == 0)
